    ...
    fetch('http://localhost:3000/articoli')
        ...
        .then((data) => {
            ...
            array.map((element: any) => {
                let capoAbbigliamento = new Articolo(element.id, element.codprod, element.collezione, element.capo, element.modello, element.quantita, element.colore, element.prezzoivaesclusa, element.prezzoivainclusa, element.disponibile, element.saldo);
                console.log(capoAbbigliamento);
                console.log('Prezzo finale prodotto: ' + capoAbbigliamento.getAcquisto().toFixed(2));
            });
